                ...
                if (minimum_order_amount_check && $('#sub_total').val() < minimum_order_amount) {
                    ...
                    if (offline_payment_active == '1' && $('.offline_payment_option').is(":checked") && $('#trx_id')
                        ...
                        if (allIsOk) {
                            ...
                            const totalAmount = document.getElementById('totalAmount').textContent.split(".")[0].replace(/[^\d]/g, "");
                            const phoneNumber = document.getElementById('phoneNumber').textContent.replace("+", "");
                            
                            const formData = new FormData();       // pass form to FormData
                            formData.append('totalAmount', totalAmount); // ✅ Add to fetch body
                                formData.append('phoneNumber', phoneNumber); // ✅ Add to fetch body

                            const selected = document.querySelector('input[name="payment_option"]:checked');
                            if (selected.value === "MTN" || selected.value === "Airtel" ) {
                                
                                // $('meta[name="csrf-token"]').attr('content')
                                //send ajax request to initiate Yo
                                fetch("{{ route('yopayments.initiate') }}", {
                                    method: 'POST',
                                    headers: {
                                ...
                                .catch(err => {
                                    ...
                                    .then(data => {
                                        ...
                                        //     $('#checkout-form').submit();
                                        // }else{
                                            AIZ.plugins.notify('danger', data.token);
